...
PKG_NAME="Banshee-Plugins-Lyrics"
...
function check_autotool_version () {
	which $1 &>/dev/null || {
		error "$1 is not installed, and is required to configure $PKG_NAME"
	}
	...
	if [ x"$do_bail" = x"yes" ]; then
		error "$1 version $2 or better is required to configure $PKG_NAME"
	fi
}
...
function run () {
	...
	$@ 2>.autogen.log || {
		cat .autogen.log 1>&2
		rm .autogen.log
		error "Could not run $1, which is required to configure $PKG_NAME"
	}
	rm .autogen.log
}
